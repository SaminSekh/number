document.addEventListener('DOMContentLoaded', () => {
    ...
    async function generateNumbers() {
        ...
        if (isHighQuality || currentMode !== 'general' || REAL_ACTIVE_SERIES[countryCode]) {
            ...
            const batchSize = qty > 10000 ? 5000 : 500;

            for (let i = 0; i < qty; i++) {
                // Optimized yielding: yield only every batchSize numbers
                if (i > 0 && i % batchSize === 0) {
                    generateBtn.innerHTML = `<i class="ph ph-spinner ph-spin"></i> ${i} / ${qty}`;
                ...
                generatedNumbers.push(createNumberObj(generatedNumbers.length + 1, fullNum, `Verified Active (${opLabel})`, countryCode));
            }
    ...
    async function generateBatchWithRandomPrefixes(qty, params) {
        ...
        const countryCode = countrySelect.value;
        const chunkSize = qty > 50000 ? 10000 : 2000;
        ...
        while (generatedNumbers.length < qty && currentAttempts < maxTotalAttempts) {
            ...
            for (let i = 0; i < batchLimit && currentAttempts < maxTotalAttempts; i++) {
                ...
                let isValid = false;
                try {
                    // For very large batches, we use a faster heuristic if libphonenumber is too slow
                    if (qty > 100000) {
                        isValid = fullNumber.length >= 8; // Simple length check for extreme speed
                    } else {
                ...
                if (isValid) {
                    generatedNumbers.push(createNumberObj(generatedNumbers.length + 1, fullNumber, 'Valid', countryCode));
                } else {
                    i--;
                }
            }
    ...
    // --- Optimization: Prefix Cache ---
    const OPERATOR_CACHE = new Map();

    function getOperatorInfo(countryCode, number) {
        if (!OPERATOR_CACHE.has(countryCode)) {
            const pattern = MOBILE_PATTERNS[countryCode];
            if (!pattern || !pattern.premium) {
                OPERATOR_CACHE.set(countryCode, null);
            } else {
                const index = new Map();
                for (const [name, prefs] of Object.entries(pattern.premium)) {
                    for (const p of prefs) {
                        index.set(p, {
                            name,
                            isPrime: pattern.highQuality ? pattern.highQuality.includes(p) : false
                        });
                    }
                }
                OPERATOR_CACHE.set(countryCode, index);
            }
        }

        const index = OPERATOR_CACHE.get(countryCode);
        if (!index) return null;

        const clean = number.replace(/\D/g, '');
        // Typical prefixes are 1 to 5 digits. We check against the index.
        // For performance, we'll try common prefix lengths.
        for (let len = 5; len >= 1; len--) {
            // We search for the prefix in the cleaned number. 
            // Usually it's right after the country code.
            // Heuristic search: check first 8 digits for any of our prefixes
            const chunk = clean.substring(0, 8);
            for (const [p, info] of index.entries()) {
                if (chunk.includes(p)) {
                    return info;
                }
            }
            break; // We only need one pass with this heuristic
        }
        return null;
    }

    function createNumberObj(id, number, status = 'Generated', countryCode) {
        let finalStatus = status;
        const info = getOperatorInfo(countryCode, number);

        if (info) {
            const quality = info.isPrime ? 'High' : 'Medium';
            finalStatus = `Valid (${info.name} ${info.isPrime ? 'Prime' : 'Standard'}) - ${quality} Quality`;
        }
    ...
    function validateNumbersStrict() {
        ...
        setTimeout(() => {
            ...
            const isLargeDataset = originalCount > 10000;

            // 1. Remove Duplicates
            const seen = new Set();
            const uniqueNumbers = [];
            for (let i = 0; i < generatedNumbers.length; i++) {
                const item = generatedNumbers[i];
            ...
            uniqueNumbers.forEach(item => {
                try {
                    let pref;
                    if (isLargeDataset) {
                        // Fast heuristic for large datasets: just take first 6 digits
                        pref = item.number.substring(0, 6);
                    } else {
            ...
            // Only show top 50 prefixes to avoid UI lag
